...
const Walker = () => {
  ...
  const handleDeleteApi = async (id) => {
    try {
      const response = await fetch(`http://localhost:5000/api/walker/${id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          token: localStorage.getItem('token'),
  ...
  return (
    <div className='max-w-7xl mx-auto'>
      ...
      <div class='overflow-x-auto pt-4'>
        <p className='py-2 font-bold '>Registered Walker</p>
          ...
          <tbody class='whitespace-nowrap'>
            {/* {data?.map((data) => ( */}
              <tr class='hover:bg-gray-100'>
                <td class='p-4 text-[15px] text-gray-800 '>001</td>
                <td class='p-4 text-[15px] text-gray-800 '>job1@gmail.com</td>
                ...
                <td class='p-4'>
                  <button class='mr-4' title='Edit' onClick={()=>navigate('/register-walker',{state:data})}>
                    <svg
      ...
      <div className='mt-3 w-full flex items-center justify-end'>
        <button
          type='button'
          onClick={() => navigate('/register-walker')}
